...
// Retries init up to 5 times because some Intel IGC driver versions crash intermittently.
func NewXe() *Xe {
	// Probe with external C binary first — Go's address space triggers
	// an IGC JIT compiler bug ~50% of the time. The C probe binary
	// has a different layout and works 100%.
	if C.xe_probe_external() != 0 {
		log.Printf("[xe] probe failed — no Intel GPU or driver not installed")
		return nil
	}

	// C probe passed — hardware is good. Init in Go process.
	// This may still crash due to the IGC bug. If it does, Go's runtime
	// will print a segfault and exit. The CUDA path is unaffected.
	ret := C.xe_init()
	if ret != 0 {
		return nil
	}
	...
	log.Printf("[compute] Xe initialized: %s (%d MB shared)", name, vram/1024/1024)
...
func (x *Xe) Benchmark() float64 {
	// Use the Engine.MatMul path for a simple benchmark
	n := 512
	a := make([]float32, n*n)
	b := make([]float32, n*n)
	...
	if r == nil { return 0 }
	return 0 // TODO: timed benchmark
}

// MatMul via shared memory — naive for now, will optimize with SPIR-V kernel.
...
func (x *Xe) RMSNorm(data, weight []float32, eps float32) {
	// CPU for now — L0 compute kernel coming
	n := len(data)
	var ss float32
...
// XeSharedAlloc allocates Level Zero shared memory (host+device accessible).
func XeSharedAlloc(bytes int) unsafe.Pointer {
	return C.xe_shared_alloc(C.size_t(bytes))
}

// XeFree frees Level Zero memory.
func XeFree(ptr unsafe.Pointer) {
	C.xe_free(ptr)
}

// XeInitialized returns true if Level Zero is ready.
func XeInitialized() bool {
	return C.xe_initialized() == 1
}

// XeLoadSPIRV loads a SPIR-V binary and creates a compute kernel.
// Returns a shader index for dispatch, or -1 on failure.
func XeLoadSPIRV(spirvData []byte, kernelName string) int {
	cName := C.CString(kernelName)
	defer C.free(unsafe.Pointer(cName))
	return int(C.xe_load_spirv(unsafe.Pointer(&spirvData[0]), C.size_t(len(spirvData)), cName))
}

// XeDispatchRMSNorm dispatches RMSNorm on the Xe GPU.
...
// XeDispatchSiLUGate dispatches fused SiLU-gate-mul on Xe.
