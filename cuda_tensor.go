...
type Int8Tensor struct {
	DataPtr  unsafe.Pointer // INT8 data on GPU [rows * cols bytes]
	ScalePtr unsafe.Pointer // FP32 scales on GPU [rows * 4 bytes]
	Rows     int
	Cols     int
	eng      *CUDA
}

// FromHostInt8 uploads INT8 quantized data + scales to GPU.
// Uses 1 byte per weight element instead of 4 (FP32) — 4x compression.
...
// DequantToFP16 dequantizes INT8 weights to FP16 in a pre-allocated buffer.
// Returns an FP16 Tensor suitable for cuBLAS mixed-precision matmul.
// The FP16 buffer can be reused across steps (allocate once, dequant each step).
...
// DequantToFP32 dequantizes INT8 weights to FP32 in a pre-allocated buffer.
// For small tensors (norms, biases) where FP16 precision isn't sufficient.
...
// AllocFP16Buffer allocates GPU memory for FP16 dequantization output.
// Returns the raw pointer (2 bytes per element).
func (c *CUDA) AllocFP16Buffer(nElements int) unsafe.Pointer {
	return C.tw_gpu_alloc(C.size_t(nElements * 2))
}

// AllocFP16Tensor allocates a GPU tensor for FP16 data with a proper cuPtr.
// Size is in FP16 elements. NOT pool-managed — FP16 byte sizes would corrupt
// the FP32 pool (poolKey mismatch → buffer overread on reuse → segfault).
// Caller must free with FreeFP16Tensor, not Release.
func (c *CUDA) AllocFP16Tensor(nElements int, shape []int) *Tensor {
	fp16Bytes := nElements * 2
	ptr := C.tw_gpu_alloc(C.size_t(fp16Bytes))
	return &Tensor{
		Shape:  shape,
		Size:   nElements,
		device: &cuPtr{ptr: ptr},
		// eng intentionally nil — prevents accidental Release into FP32 pool
	}
}

// FreeFP16Tensor directly frees an FP16 tensor's GPU memory.
...
// ReleaseInt8 frees GPU memory for an Int8Tensor.
func (c *CUDA) ReleaseInt8(q *Int8Tensor) {
	if q.DataPtr != nil {
		C.tw_gpu_free(q.DataPtr)
		q.DataPtr = nil
	}
	if q.ScalePtr != nil {
		C.tw_gpu_free(q.ScalePtr)
		q.ScalePtr = nil
	}
}

// VRAMBytesInt8 returns VRAM usage of an Int8Tensor.
func (q *Int8Tensor) VRAMBytes() int {
	return q.Rows*q.Cols + q.Rows*4 // data + scales
}
